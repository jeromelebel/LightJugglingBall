#include "LIS331Accelerometer.h"
#include "i2c.h"
#include <Arduino.h>

LIS331Accelerometer::LIS331Accelerometer(LIS331Address address, LIS331ScaleRange fullScaleRange, LIS331DataRate dataRate)
{
    _address = address;
...
unsigned char LIS331Accelerometer::init()
{
    ...
    if (c == 0x2A) {
        ...
        if ((_fullScaleRange == LIS331ScaleRange2g) || (_fullScaleRange == LIS331ScaleRange4g) || (_fullScaleRange == LIS331ScaleRange8g)) {
        ...
        if (_dataRate <= LIS331DataRate1Hz) {
...
void LIS331Accelerometer::readRawData(short *destination)
{
...
void LIS331Accelerometer::convertRawDataIntoAccelData(short *rawVector, float *vector)
{
    int ii;
...
/* Sets the LIS331 to standby mode.
   It must be in standby to change most register settings */
void LIS331Accelerometer::setStandbyMode(void)
{
    unsigned char c = this->readRegister(0x2A);
    this->writeRegister(0x2A, c & ~(0x01));
}

/* Sets the LIS331 to active mode.
   Needs to be in this mode to output data */
void LIS331Accelerometer::setActiveMode(void)
{
    unsigned char c = this->readRegister(0x2A);
    this->writeRegister(0x2A, c | 0x01);
}
...
void LIS331Accelerometer::readRegisters(unsigned char address, int i, unsigned char * dest)
{
    i2cSendStart();
    i2cWaitForComplete();
...
unsigned char LIS331Accelerometer::readRegister(unsigned char address)
{
    unsigned char data;
    byte x = address;
...
void LIS331Accelerometer::writeRegister(unsigned char address, unsigned char data)
{
    i2cSendStart();
    i2cWaitForComplete();
